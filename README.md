...
本プロジェクトではAI（Gemini/Copilot）をコーディングパートナーとして活用しましたが、**インフラ構築とトラブルシューティングにおいては、エンジニア自身の判断と粘り強い調査**がプロジェクトの成否を分けました。

### 1. コンテナ間ネットワークとデータ消失危機の克服
Docker環境で「Frontend, Backend, DB, Cloudflare Tunnel, n8n」を連携させる際、最も苦心したのがネットワーク設計です。

* **課題:** コンテナ間の名前解決ができず、無理にネットワーク設定を変更した結果、n8nのボリュームマウントが外れ、**ワークフローデータが初期化されかけるインシデント**が発生しました。
* **解決:** AIに頼らず、公式ドキュメントに基づきDocker Network（Bridge）の仕様を再確認。コンテナログを1つずつ追跡し、正しいボリュームの再アタッチと、セキュアな内部通信網の確立を自分の手で完遂しました。

### 2. AIとの協働における「主導権」の維持
* **Coding:** エラー解析や定型コードの生成にはAIを積極活用し、開発速度を約3倍に向上させました。
* **Decision:** ただし、インフラ構成やセキュリティ要件（どのポートを閉じ、どのデータを永続化するか）の決定権は常に自分に持ち、AIが提案する「動くけど危険な設定（例: 全ポート開放）」は断固として採用しませんでした。
...
| 翻訳リーダー | モバイル対応 |
| :---: | :---: |
| <img src="https://github.com/user-attachments/assets/6b819400-df60-4f39-ba32-196990bab2e8" width="400" alt="翻訳画面" /> | <img src="https://github.com/user-attachments/assets/18a10ef0-9020-417b-810a-0549b308b6b3" width="400" alt="スマホ画面"/> |
