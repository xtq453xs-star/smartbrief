...
import jp.smartbrief.billing.domain.Work;
import jp.smartbrief.billing.dto.BookResponse;
import jp.smartbrief.billing.repository.UserBookHistoryRepository;
import jp.smartbrief.billing.repository.UserRepository;
import jp.smartbrief.billing.repository.WorkRepository;
...
public class LineController {
        ...
        return userRepository.findByUsername(request.getUsername())
                ...
                .flatMap(user -> {
                    ...
                    return userRepository.save(user)
                            .map(saved -> ResponseEntity.ok(Objects.requireNonNull(Map.of("message", "連携に成功しました！\nWebのプレミアム機能がLINEでも有効になります。"))));
                })
    ...
    public Mono<ResponseEntity<Object>> readBook(@RequestBody ReadRequest request) {
        // lineUserId も null の可能性を排除
        return userRepository.findByLineUserId(Objects.requireNonNull(request.getLineUserId()))
                .switchIfEmpty(Mono.error(new RuntimeException("NOT_LINKED")))
                .flatMap(user -> {
                    // ★修正箇所: 67行目 request.getBookId() を requireNonNull でラップ
                    return workRepository.findById(Objects.requireNonNull(request.getBookId()))
                        .flatMap(work -> {
                            // プレミアム判定
                                    ...
                                    .flatMap(count -> {
                                        if (count >= 3) {
                                            return Mono.just(ResponseEntity.status(HttpStatus.FORBIDDEN)
                                                    .body((Object)Objects.requireNonNull(Map.of("message", "無料プランの1日の閲覧制限（3回）に達しました。\n\nWebでプレミアムプランに登録すると無制限で読めます！\n👇\nhttps://smartbrief.jp"))));
                                        }
                                        // 制限内
                ...
                .onErrorResume(e -> {
                    if ("NOT_LINKED".equals(e.getMessage())) {
                        return Mono.just(ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                                .body(Objects.requireNonNull(Map.of("message", "アカウントが連携されていません。\nメニューの「連携する」からログイン情報を入力してください。"))));
                    }
                    return Mono.just(ResponseEntity.status(HttpStatus.NOT_FOUND)
                            .body(Objects.requireNonNull(Map.of("message", "作品が見つかりません。"))));
                });
    }
    ...
    private Mono<ResponseEntity<Object>> recordHistoryAndResponse(User user, Work work, boolean isPremium) {
        ...
        // ★修正: ID取得部分も安全に
        return historyRepository.findFirstByUserIdAndBookIdOrderByViewedAtDesc(Objects.requireNonNull(user.getId()), Objects.requireNonNull(work.getId()))
    ...
    private Mono<ResponseEntity<Object>> saveNewHistory(User user, Work work, boolean isPremium) {
        ...
        history.setUserId(Objects.requireNonNull(user.getId()));
